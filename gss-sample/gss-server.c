...
/* TODO(VSY): Make this determination via configure */
...
fprintf(stderr, "ERROR IMPORTING NAME for (%s)\n", service_name);
fflush(stderr);
        display_status("importing name", maj_stat, min_stat);
        return -1;
    }
fprintf(stderr, "AFTER IMPORTING NAME\n");
fflush(stderr);
...
fprintf(stderr, "AFTER ACQUIRING CRED\n");
fflush(stderr);
